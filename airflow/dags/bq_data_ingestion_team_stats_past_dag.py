...
parquet_file = "teams_game_stats_till_last_year.parquet"
...
def format_to_parquet():
    ...
    for season in Seasons:
        try:
            teamlogdata = teamgamelogs.TeamGameLogs(season_nullable=season)
            teamlogdata = teamlogdata.team_game_logs.get_data_frame()
            teamlogdata = pd.DataFrame(teamlogdata)
            teamlogdata[["GAME_DATE"]] = teamlogdata[["GAME_DATE"]].apply(
                pd.to_datetime
            )
            team_history_data = pd.concat(
                [team_history_data, teamlogdata], ignore_index=True
            )
            print(season + " - " + str(len(teamlogdata)) + " rows")
        except Exception as e:
            print("skipped" + season + "because of" + e)

    team_history_data = team_history_data.loc[
        (team_history_data["GAME_DATE"].dt.date < current_season_first_game_date)
    ]
    team_history_data.to_parquet(parquet_file)
...
with DAG(
    dag_id="bq_teams_stats_past_dag",
